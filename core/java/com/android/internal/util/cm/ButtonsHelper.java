...
public class ButtonsHelper {
    ...
    public static ArrayList<ButtonConfig> getNavBarConfigWithDescription(Context context, String values, String entries) {
        return (getButtonsConfigValues(context,
    ...
    public static void setNavBarConfig(Context context, ArrayList<ButtonConfig> buttonsConfig, boolean reset) {
        String config;
        if (reset) {
            config = ButtonsConstants.NAVIGATION_CONFIG_DEFAULT;
        } else {
    ...
    public static ArrayList<ButtonConfig> getPieConfigWithDescription(Context context, String values, String entries) {
        return (getButtonsConfigValues(context,
    ...
    public static void setPieConfig(Context context, ArrayList<ButtonConfig> buttonsConfig, boolean reset) {
        String config;
        if (reset) {
            config = ButtonsConstants.NAVIGATION_CONFIG_DEFAULT;
        } else {
    ...
    public static ArrayList<ButtonConfig> getPieSecondLayerConfigWithDescription(Context context, String values, String entries) {
        return (getButtonsConfigValues(context,
    ...
    public static ArrayList<ButtonConfig> getNavRingConfigWithDescription(Context context, String values, String entries) {
        return (getButtonsConfigValues(context,
    ...
    public static void setNavRingConfig(Context context, ArrayList<ButtonConfig> buttonsConfig, boolean reset) {
        String config;
        if (reset) {
            config = ButtonsConstants.NAV_RING_CONFIG_DEFAULT;
        } else {
    ...
    //public static void setLockscreenShortcutConfig(Context context, ArrayList<ButtonConfig> buttonsConfig, boolean reset) {
    //    String config;
    //    if (reset) {
    //        config = "";
    //    } else {
        ...
        for (String configValue : config.split("\\" + ButtonsConstants.ACTION_DELIMITER)) {
            ...
            if (counter == 1) {
                buttonConfig = new ButtonConfig(configValue,
                            getProperSummary(pm, settingsResources, configValue, values, entries), null, null, null);
            }
            if (counter == 2) {
                if (isShortcut) {
                    ...
                    buttonConfig.setLongpressActionDescription(
                                getProperSummary(pm, settingsResources, configValue, values, entries));
                }
            }
            if (counter == 3) {
                buttonConfig.setIcon(configValue);
    ...
    private static String setButtonsConfig(ArrayList<ButtonConfig> buttonsConfig, boolean isShortcut) {
        String finalConfig = "";
        ...
        for (int i = 0; i < buttonsConfig.size(); i++) {
            ...
            if (!isShortcut) {
                finalConfig += buttonConfig.getLongpressAction() + ButtonsConstants.ACTION_DELIMITER;
            }
    ...
    public static Drawable getButtonIconImage(Context context, String clickAction, String customIcon) {
        int resId = -1;
        PackageManager pm = context.getPackageManager();
        if (pm == null) {
            return null;
    ...
    private static int getButtonsSystemIcon(Resources systemUiResources, String clickAction) {
        ...
        if (clickAction.equals(ButtonsConstants.ACTION_HOME)) {
            ...
            resId = systemUiResources.getIdentifier(
                        SYSTEMUI_METADATA_NAME + ":drawable/ic_navbar_silent", null, null);
    ...
    private static String getFriendlyActivityName(PackageManager pm, Intent intent, boolean labelOnly) {
